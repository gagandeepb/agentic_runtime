defmodule AgenticRuntime.Agents.AgentPersistence do
  ...
  def persist_state(scope, state_data, context) do
    ...
    case AgenticRuntime.Conversations.save_agent_state(scope, conversation_id, state_data) do
      {:ok, _} ->
  ...
  def load_state(scope, context) do
    conversation_id = extract_conversation_id(context.agent_id)
    AgenticRuntime.Conversations.load_agent_state(scope, conversation_id)
  end
